...
export default function Controls({ wheels, decks, metals, className }: Props) {
  ...
  return (
    <div className={clsx("flex flex-col gap-6", className)}>
      <Options title="Shape/Deck" selectedName={selectedDeck?.uid}>
        {decks.map((deck) => (
          <Option
            key={deck.uid}
      ...
      <Options title="Rodas" selectedName={selectedWheel?.uid}>
        {wheels.map((wheel) => (
          <Option
            key={wheel.uid}
      ...
      <Options title="Trucks/Eixos" selectedName={selectedTruck?.uid}>
        {metals.map((metal) => (
          <Option
            key={metal.uid}
      ...
      <Options title="Parafusos" selectedName={selectedBolt?.uid}>
        {metals.map((metal) => (
          <Option
            key={metal.uid}
            colorField={metal.color}
